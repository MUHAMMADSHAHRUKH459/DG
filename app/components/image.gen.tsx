...
const Index = () => {
  ...
  const generateImage = async () => {
    ...
    try {
      // ✅ Correct API call to YOUR Next.js backend
      const response = await axios.post(
        "/api/generate",  // Your local API route (matches route.ts)
        { prompt },
        {
          headers: {
      ...
      if (response.data.imageUrl) {
        setUploadedImage(response.data.imageUrl); // Directly use the URL
      ...
      <motion.div className="max-w-lg w-full glass-card rounded-3xl overflow-hidden p-6 sm:p-8">
        ...
        <div className="mb-6">
          {!uploadedImage ? (
            ...
            <div className="relative">
              ...
              <button onClick={() => setUploadedImage(null)} className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1.5 hover:bg-black/70">
                <X size={16} />
              </button>
            </div>
          )}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {suggestedPrompts.map((prompt, index) => (
            <motion.div key={index} className={`px-3 py-1 text-xs sm:text-sm rounded-full cursor-pointer border ${selectedPrompt === prompt ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`} onClick={() => handlePromptSelect(prompt)}>
              {prompt}
            </motion.div>
          ))}
        </div>
        ...
        <div className="mt-4">
          <input type="text" value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} className="w-full px-4 py-2 border rounded-lg text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-800 text-black" placeholder="Enter your custom prompt..." />
